...
struct RDValue {
 ...
 inline RDValue(): type(Empty) {}
  // Pod Style (Direct storage)
 inline RDValue(double v)   : value(v), type(Double) {}
 inline RDValue(float v)    : value(v), type(Float) {}
 inline RDValue(int v)      : value(v), type(Int) {}
 inline RDValue(unsigned v) : value(v), type(UnsignedInt) {}
 inline RDValue(bool v)     : value(v), type(Bool) {}

 inline RDValue(boost::any *v)  : value(v),type(Any) {}

  // Copies passed in pointers
 inline RDValue(const boost::any &v)  : value(new boost::any(v)),type(Any) {}
 inline RDValue(const std::string &v) : value(new std::string(v)),type(String){};
 template <class T>
 inline RDValue(const T &v) : value(new boost::any(v)),type(Any) {}
