...
bool isUnsaturated(const Atom *atom, const ROMol &mol) {
  for(auto bond : atom->bonds()) {    // can't just check for single bonds, because dative bonds also have an
    // order of 1
...
void dfsFindCycles(ROMol &mol, int atomIdx, int inBondIdx,
  ...
  possibles.reserve(atom->bonds().size());
  for(auto theBond: atom->bonds()) {
  ...
  for (auto &msI : molStack) {
    if (msI.type == MOL_STACK_BOND) {
      ...
      if (canHaveDirection(*tBond) && bondDirCounts[tBond->getIdx()] >= 1) {
        ...
        for(auto nbnd: canonBeginAtom->bonds()) {
          if (nbnd != tBond && nbnd->getBondType() == Bond::DOUBLE &&
              nbnd->getStereo() > Bond::STEREOANY) {
            dblBondAtom =
        ...
        for(auto nbnd: canonEndAtom->bonds()) {
          if (nbnd != tBond && nbnd->getBondType() == Bond::DOUBLE &&
              nbnd->getStereo() > Bond::STEREOANY) {
